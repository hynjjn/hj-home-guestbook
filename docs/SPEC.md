...
**`id`는 내부 식별자로만 쓴다. 화면에는 번호를 안 보여준다.**

처음에는 `id`를 화면 번호로 그대로 썼다. "soft delete라 번호가 비지 않는다"고 봤는데 틀렸다. 행은 남지만 피드가 `deleted_at IS NULL`로 걸러내므로 화면에서는 그대로 빈다. 1번과 3번만 남고 2번이 사라진 채로 보인다.

번호를 연속되게 만드는 방법은 두 가지인데 둘 다 값이 비쌌다.

조회할 때마다 세면 연속은 되지만 삭제가 일어날 때마다 뒤 번호가 전부 밀리고, 무엇보다 `id`와 영영 어긋난다. 관리 화면 없이 전부 SQL로 손보는 구조에서 화면이 46번인데 행이 47번이면 매번 옮겨 적어야 한다. avatar seed도 `id`에서 나오므로 표시되는 번호와 얼굴이 서로 다른 걸 가리키게 된다.

삭제된 자리에 "삭제된 글" 표시를 남기면 연속성과 안정성을 둘 다 얻지만, **검열한 사실이 그대로 드러난다.** 주인장이 글을 내리는 일이 화면에 남는 건 내리는 이유와 정면으로 어긋난다.

번호를 안 보여주면 셋 다 안 생긴다. 잃는 건 "NO.47"이 주던 장부 느낌뿐이고, 그건 기능이 아니라 장식이다. `id`는 API 호출과 avatar seed로만 쓴다.
